...
- **The "describe-instead-of-execute" trap — the failure that drove the biggest design change.** Issue #32 (replace a broken URL) was the perfect typo task: Claude found the 3 matching files via `search_content`, then ended the turn with *"I found these files, let me prepare the updated content for each"* and never called `propose_edit`. Tightening the prompt made Claude more cautious instead of more decisive — it just burned all 15 iterations on repeated `read_file` + narration, hitting the 5-minute action timeout. 90 minutes of iteration, zero PRs. Full writeup in the Appendix. **Lesson I actually shipped:** pure find-and-replace shouldn't use an agent at all. I wrote a 130-line `deterministic-fix.js` that regex-extracts the old/new pair from the issue body and greps the repo — the same #32 now ships as [adobe/aio-lib-events#83](https://github.com/adobe/aio-lib-events/pull/83) in 10 seconds, zero LLM calls. Claude is preserved as a fallback for issues where regex extraction can't find a clean pair. **The meta-lesson:** prompt engineering is not a reliable safety mechanism against a strong LLM reflex; pick the right tool for the shape of the problem before reaching for an agent.
...
**Pick the right tool for the shape of the problem before reaching for an agent.** The single biggest failure of the week (issue #32 — never committed a fix) was a find-and-replace task. A deterministic grep-and-rewrite helper produced the same PR in under 10 seconds; the Claude tool-use loop had burned 15 iterations and a 5-minute action timeout on "let me continue reading." I shipped the deterministic-first path on Day 4 (see `actions/common/deterministic-fix.js`) — #32 is now [adobe/aio-lib-events#83](https://github.com/adobe/aio-lib-events/pull/83). Going forward: for every new class of task, ask *"does this have a clean mechanical solution?"* before writing a system prompt. Use the agent as the fallback, not the default.
...
**Day 3 — the roadblock that became the lesson: issue #32 (the rxjs-dev URL replacement).**

> **Update Day 4:** the original version of this section documented #32 as an unresolved failure. After writing up the "pick the right tool for the shape of the problem" reflection, I went back and *implemented* that lesson as a deterministic find-and-replace path in `actions/common/deterministic-fix.js`. Issue #32 now ships cleanly as [adobe/aio-lib-events#83](https://github.com/adobe/aio-lib-events/pull/83) — 3 files modified, draft PR opened, under 10 seconds per run, no LLM call needed for the actual fix. The failure writeup below is kept intact as the first half of the story; the resolution at the bottom is the second half.
...
- **Honest status reporting matters more than a green demo.** The original version of this reflection said *"#32 remains skipped; the dead-end is the lesson."* Then I implemented the lesson. The deterministic path is ~130 lines in `actions/common/deterministic-fix.js`: regex-extract an `(old, new)` pair from the issue body (prioritising the aio issue template's Expected/Actual sections), grep the repo, rewrite matching files, commit. Returns `null` and falls through to the Claude tool-use loop whenever extraction doesn't yield a confident pair — so the LLM is preserved for genuinely unstructured cases and bypassed for mechanical ones. The exact same issue that Claude couldn't solve in four attempts now ships a real draft PR in about 10 seconds. That's the actual lesson: the best thing to do with a failing-agent transcript is treat it as a spec for the non-agent fallback.
...
- **Real dep-bump archetype** — ✅ shipped Day 4. New `actions/common/dep-bump.js` extracts (package, fromVersion, toVersion) from the issue body via four regex patterns (Dependabot-style, explicit from-to, target-only, simple-bump). Queries `registry.npmjs.org` for the latest version when the target isn't specified. Rewrites `package.json` with preserved caret/tilde range prefix. Returns null on extraction or registry-lookup failure so the issue is cleanly marked skipped. All six unit-test cases pass (caret preservation, tilde preservation, scoped names, simple registry lookup, package-not-in-pkg-json, unextractable). Live end-to-end against the watched aio repos is pending a real dep-bump *issue* appearing (all open bumps in those repos today are Dependabot *PRs*, which we correctly filter out of the issue feed).
- **Deterministic-first for typo archetype** — ✅ shipped Day 4 (see `actions/common/deterministic-fix.js`).
